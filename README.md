# Buildify - AI Agents SaaS Website Template

#### Preview

 - [Demo](https://themewagon.github.io/buildify-reactjs/)

#### Download
 - [Download from ThemeWagon](https://themewagon.com/themes/buildify-reactjs/)

## Getting Started

1. Clone Repository
```
git clone https://github.com/themewagon/buildify-reactjs.git
```
2. Install Dependencies
```
npm i
```
3. Run the development server:

```bash
npm run dev
# or
yarn dev
# or
pnpm dev
# or
bun dev
```

## Running with Docker

1. Build and start the container:
```bash
docker-compose up -d
```
2. Access the application at `http://localhost:8085`


## Author 
```
Design and code is completely written by PrebuiltUI and development team. 
```

## License

 - Design and Code is Copyright &copy; <a href="https://prebuiltui.com/" target="_blank">PrebuiltUI</a>
 - Licensed cover under [MIT]
 - Distributed by <a href="https://themewagon.com" target="_blank">ThemeWagon</a>
